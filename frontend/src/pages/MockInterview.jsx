...
const Icon = {
  Bot: (p) => (
    <svg {...p} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <rect width="18" height="12" x="3" y="11" rx="2"/><circle cx="12" cy="5" r="2"/><path d="M12 7v4"/><line x1="8" x2="8" y1="16" y2="16"/><line x1="16" x2="16" y1="16" y2="16"/>
    </svg>
  ),
  Mic: (p) => (
    <svg {...p} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
export default function MockInterview() {
  ...
  const speakText = useCallback((text) => {
    ...
    const v = voices.find(v => v.lang.startsWith("en") && (v.name.includes("Natural") || v.name.includes("Google") || v.name.includes("Samantha"))) || voices.find(v => v.lang.startsWith("en"));
    if (v) u.voice = v;
  ...
  const startListening = useCallback(() => {
    if (!SpeechRecognition) { alert("Voice recognition requires Chrome or Edge."); return; }
    stopSpeaking();
    try {
      ...
      r.onerror = (e) => { if (e.error === "not-allowed") alert("Microphone permission denied."); setIsListening(false); };
  ...
  const handleFileSelect = (f) => {
    ...
    if (!n.endsWith(".pdf") && !n.endsWith(".docx")) { setSetupError("Please select a PDF or DOCX file."); return; }
    setFile(f); setSetupError("");
  };
  ...
  const handleStart = async () => {
    setSetupError("");
    if (!jobRole.trim()) { setSetupError("Please select or enter a target job role."); return; }
    if (!file) { setSetupError("Please upload your resume (PDF or DOCX)."); return; }
    setStarting(true);
    try {
      const fd = new FormData();
      fd.append("job_role", jobRole.trim());
      fd.append("file", file);
      const data = await djangoRequest("/api/resume/interview/start/", { method: "POST", body: fd });
      if (!data?.interview_id) throw new Error(data?.error || "Failed to start session.");
      setInterviewId(data.interview_id);
      setQuestions(data.questions || []);
      setCurrentIdx(0); setAnswer(""); setFeedback(null); setAllFeedback([]);
      setStep("interview");
    } catch (err) {
  ...
  const handleSubmitAnswer = async () => {
    ...
    try {
      ...
      const data = await djangoRequest(`/api/resume/interview/${interviewId}/answer/`, { method: "POST", body: { question_id: q.id, answer: answer.trim() } });
  ...
  const handleNext = async () => {
    ...
    if (currentIdx === questions.length - 1) { await handleFinish(); }
    else { setCurrentIdx(i => i + 1); setAnswer(""); setFeedback(null); }
  };

  const handleFinish = async () => {
    stopSpeaking(); stopListening(); setFinishing(true);
    try {
        ...
        <div style={{ maxWidth: 700, margin: "0 auto" }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20, flexWrap: "wrap", gap: 8 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <span style={{ width: 8, height: 8, borderRadius: "50%", background: isListening ? "#ef4444" : isSpeaking ? "#4f46e5" : "#22c55e", display: "inline-block", flexShrink: 0 }} />
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <button
                type="button"
                onClick={() => { if (voiceMode) stopSpeaking(); setVoiceMode(v => !v); }}
                style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12.5, fontWeight: 500, background: "#fff", color: voiceMode ? "#4f46e5" : "#64748b", border: `1px solid ${voiceMode ? "#c7d2fe" : "#e2e8f0"}`, padding: "5px 10px", borderRadius: 8, cursor: "pointer" }}
              >
                {voiceMode ? <Icon.Volume width="14" height="14" /> : <Icon.VolumeX width="14" height="14" />}
                Voice {voiceMode ? "on" : "off"}
              </button>
              <span style={{ fontSize: 12, color: "#94a3b8", background: "#f1f5f9", padding: "5px 10px", borderRadius: 8, fontWeight: 500 }}>{jobRole}</span>
            </div>
          </div>
          ...
          <div
            ...
            style={{ background: "#ffffff", borderRadius: 18, padding: "clamp(22px,4vw,36px)", boxShadow: "0 2px 16px rgba(15,23,42,0.05)", border: "1px solid #e2e8f0", marginBottom: 16 }}
          >
            {/* AI interviewer header */}
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18, paddingBottom: 16, borderBottom: "1px solid #f1f5f9" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <div style={{ width: 36, height: 36, borderRadius: 10, background: "linear-gradient(135deg,#4f46e5,#6366f1)", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", flexShrink: 0 }}>
                  <Icon.Bot width="16" height="16" />
                </div>
                <div>
                  <div style={{ fontSize: 13, fontWeight: 700, color: "#0f172a" }}>AI Interviewer</div>
                  {isSpeaking && (
              ...
              <button
                ...
                style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 12.5, fontWeight: 500, background: "transparent", color: isSpeaking ? "#dc2626" : "#4f46e5", border: "none", padding: "5px 8px", cursor: "pointer", borderRadius: 6 }}
              >
                {isSpeaking ? <Icon.VolumeX width="15" height="15" /> : <Icon.Volume width="15" height="15" />}
                {isSpeaking ? "Stop" : "Play"}
              </button>
            </div>

            {/* Question */}
            <p style={{ fontSize: "clamp(15px,2.5vw,18px)", fontWeight: 600, color: "#0f172a", lineHeight: 1.55, margin: "0 0 22px", letterSpacing: "-0.01em" }}>
              {currentQ.question}
            </p>
            ...
            {!feedback && (
              <>
                {/* Voice control */}
                <div style={{ display: "flex", alignItems: "center", gap: 10, background: isListening ? "#fef2f2" : "#f8fafc", border: `1px solid ${isListening ? "#fecaca" : "#e2e8f0"}`, borderRadius: 10, padding: "10px 14px", marginBottom: 12, transition: "all 0.2s" }}>
                  <button
                    type="button"
                  ...
                  >
                    {isListening ? <Icon.MicOff width="16" height="16" /> : <Icon.Mic width="16" height="16" />}
                  </button>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 13, fontWeight: 600, color: isListening ? "#dc2626" : "#334155" }}>
                      {isListening ? "Recording…" : "Voice input"}
                    </div>
                    <div style={{ fontSize: 11.5, color: "#94a3b8" }}>
                      {isListening ? "Speak clearly. Click again to stop." : "Click mic to speak your answer"}
                    </div>
                  </div>
                  {isListening && (
                    <button
                      ...
                      style={{ fontSize: 12, fontWeight: 600, color: "#dc2626", background: "#fee2e2", border: "none", borderRadius: 7, padding: "4px 10px", cursor: "pointer" }}
                    >
                      Done
                    </button>
                  )}
              ...
              <div style={{ animation: "fadeIn 0.3s ease" }}>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(100px, 1fr))", gap: 10, marginBottom: 16 }}>
                  {[
                    ...
                    <div key={label} style={{ background: scoreBg(val), border: `1px solid ${scoreBorder(val)}`, borderRadius: 10, padding: "12px", textAlign: "center" }}>
                      <div style={{ fontSize: 20, fontWeight: 800, color: scoreColor(val), lineHeight: 1 }}>{val}<span style={{ fontSize: 10, fontWeight: 600 }}>/10</span></div>
                ...
                <div style={{ background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: 10, padding: "12px 14px", marginBottom: 16 }}>
                  <div style={{ fontSize: 12, fontWeight: 700, color: "#334155", marginBottom: 4, textTransform: "uppercase", letterSpacing: "0.04em" }}>Feedback</div>
                  <div style={{ color: "#475569", fontSize: 13.5, lineHeight: 1.6 }}>{feedback.tip}</div>
                </div>
                ...
                >
                  {finishing ? <><span className="spinner" />Generating summary…</> : isLast ? <>Finish Interview <Icon.Check width="15" height="15" /></> : <>Next Question <Icon.ArrowRight width="15" height="15" /></>}
                </button>
              </div>
            )}

            {feedback?.error && (
              <div style={{ background: "#fef2f2", border: "1px solid #fecaca", color: "#dc2626", padding: "10px 14px", borderRadius: 10, fontSize: 13, display: "flex", gap: 8, alignItems: "center" }}>
                <Icon.AlertTriangle width="15" height="15" style={{ flexShrink: 0 }} />
                {feedback.error}
              </div>
            )}
              ...
              {questions.map((_, i) => (
                <div
                  ...
                  style={{
                    ...
                    height: 7,
                    borderRadius: 99,
                    background: i < currentIdx ? "#4f46e5" : i === currentIdx ? "#4f46e5" : "#e2e8f0",
    ...
    return (
      <div style={{ ...page }}>
        <div style={{ maxWidth: 700, margin: "0 auto" }} className="card-enter">
          ...
          {/* Verdict */}
          <div style={{ background: "#ffffff", borderRadius: 16, padding: "22px 24px", marginBottom: 16, border: "1px solid #e2e8f0" }}>
            <h3 style={{ fontSize: 13.5, fontWeight: 700, color: "#64748b", margin: "0 0 10px", textTransform: "uppercase", letterSpacing: "0.06em" }}>Assessment Verdict</h3>
            <p style={{ color: "#0f172a", lineHeight: 1.7, margin: 0, fontSize: 14.5 }}>{summary?.verdict}</p>
          </div>

          {/* Strengths & improvements */}
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 14, marginBottom: 16 }}>
            <div style={{ background: "#ffffff", borderRadius: 14, padding: "20px 22px", border: "1px solid #bbf7d0" }}>
              <h4 style={{ fontSize: 13, fontWeight: 700, color: "#15803d", margin: "0 0 12px", display: "flex", alignItems: "center", gap: 7, textTransform: "uppercase", letterSpacing: "0.05em" }}>
                <Icon.Check width="14" height="14" /> Key Strengths
              </h4>
              <ul style={{ margin: 0, paddingLeft: 16 }}>
                {(summary?.strengths || []).map((s, i) => (
                  <li key={i} style={{ color: "#374151", fontSize: 13.5, lineHeight: 1.65, marginBottom: 5 }}>{s}</li>
                ))}
              </ul>
            </div>
            <div style={{ background: "#ffffff", borderRadius: 14, padding: "20px 22px", border: "1px solid #fed7aa" }}>
              <h4 style={{ fontSize: 13, fontWeight: 700, color: "#c2410c", margin: "0 0 12px", display: "flex", alignItems: "center", gap: 7, textTransform: "uppercase", letterSpacing: "0.05em" }}>
          ...
          {allFeedback.length > 0 && (
            <div style={{ background: "#ffffff", borderRadius: 16, padding: "22px 24px", marginBottom: 20, border: "1px solid #e2e8f0" }}>
              <h3 style={{ fontSize: 13.5, fontWeight: 700, color: "#64748b", margin: "0 0 14px", textTransform: "uppercase", letterSpacing: "0.06em" }}>Question Breakdown</h3>
              {allFeedback.map((item, i) => (
                <div key={i} style={{ paddingBottom: 12, marginBottom: 12, borderBottom: i < allFeedback.length - 1 ? "1px solid #f4f6fb" : "none" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12, marginBottom: 4 }}>
                    <p style={{ fontSize: 13.5, color: "#0f172a", margin: 0, fontWeight: 500, lineHeight: 1.45, flex: 1 }}>{item.question}</p>
                    <span style={{ fontWeight: 800, fontSize: 13.5, color: scoreColor(item.feedback?.overall || 0), flexShrink: 0 }}>
                      {item.feedback?.overall || 0}/10
                    </span>
                  </div>
                  <p style={{ fontSize: 12.5, color: "#64748b", margin: 0, lineHeight: 1.55 }}>{item.feedback?.tip}</p>
                </div>
              ))}
            </div>
          )}
