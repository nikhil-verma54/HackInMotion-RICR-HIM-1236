...
export function AuthProvider({ children }) {
  ...
  const sendTokenToDjango = async (firebaseUser) => {
    ...
    try {
      ...
      console.log("🐍 Django login status:", response.status);
      ...
      if (!response.ok) {
        console.error("❌ Django authentication failed");

        return null;
      }

      console.log("🟢 Firebase user successfully authenticated with Django");

      return data;
    } catch (error) {
      console.error("❌ Could not connect to Django");
  ...
  const djangoRequest = useCallback(async (endpoint, options = {}) => {
    console.log("🔵 Starting authenticated Django request...");
    ...
    try {
      ...
      console.log("📨 Authorization header prepared:", headers.has("Authorization"));
      ...
      if (!response.ok) {
        console.error("❌ Django request failed:", response.status);
      ...
      console.log("🟢 Django request successful");

      return data;
    } catch (error) {
      console.error("❌ djangoRequest failed:");
  ...
  const register = async (email, password) => {
    ...
    try {
      ...
      const djangoResponse = await sendTokenToDjango(result.user);

      if (djangoResponse) {
        console.log("✅ Django user created/updated");

        console.log("👤 Django response:", djangoResponse);
      } else {
        console.log("⚠️ Firebase registration succeeded, but Django sync failed");
      }
  ...
  const login = async (email, password) => {
    ...
    try {
      ...
      console.log("🔵 Now connecting Firebase user to Django...");
      ...
      if (djangoResponse) {
        ...
        console.log("⚠️ Firebase login succeeded, but Django authentication did not complete");
      }
  ...
  const googleLogin = async () => {
    ...
    try {
      ...
      if (djangoResponse) {
        console.log("✅ Google user authenticated with Django");
      } else {
        console.log("⚠️ Google Firebase login succeeded, but Django sync failed");
      }
